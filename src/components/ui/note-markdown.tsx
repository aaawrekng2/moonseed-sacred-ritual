...
// v3.122 — note-text cleanup (mojibake + escapes) moved to @/lib/note-text.
...
export function NoteMarkdown({
  ...
  const text = cleanNoteText(source ?? "")
    .replace(/\r\n/g, "\n")
    .replace(/\r/g, "\n");
  ...
  while (i < lines.length) {
    ...
    if (/^\s*[-*]\s+/.test(line)) {
      ...
      blocks.push(
        ...
        >
          {items.map((it, idx) => (
            <li key={idx} style={{ margin: "0.15em 0", listStyleType: "disc" }}>
              {renderInline(it)}
            </li>
          ))}
        </ul>,
    ...
    if (/^\s*\d+\.\s+/.test(line)) {
      ...
      blocks.push(
        ...
        >
          {items.map((it, idx) => (
            <li key={idx} style={{ margin: "0.15em 0", listStyleType: "decimal" }}>
              {renderInline(it)}
            </li>
          ))}
        </ol>,
