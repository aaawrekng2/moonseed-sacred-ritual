 ...
 * with Supabase-backed equivalents in subsequent phases.
 */
...
export function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="font-display text-2xl italic text-gold">{children}</h2>
  );
}
...
export function Field({
  ...
  return (
    <label className="grid gap-2">
      <span className="font-display text-sm italic text-muted-foreground-strong">
        {label}
      </span>
      {children}
    </label>
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={cn(
            "absolute top-1 h-4 w-4 rounded-full bg-gold transition",
            checked ? "left-5" : "left-1 opacity-60",
          )}
        />
      </span>
    </button>
...
export function ProfilePanel() {
  ...
  return (
    <section className="panel grid gap-5">
      ...
      <Field label="Email">
        <input
          className={cn(textInput, "text-muted-foreground")}
          value={email}
          readOnly
        />
      </Field>
      <Field label="Intention">
        <textarea
          className={cn(textInput, "min-h-28 resize-none")}
          value={intention}
          onChange={(e) => setIntention(e.target.value)}
          placeholder="What are you tending in this season?"
        />
      </Field>
    </section>
  );
...
export function PreferencesPanel() {
  ...
  const spreadOptions = useMemo(
    () => Object.keys(SPREAD_META) as SpreadMode[],
    [],
  );
  ...
  return (
    <div className="grid gap-4">
          ...
          >
            {spreadOptions.map((spread) => (
              <option key={spread} value={spread}>
                {SPREAD_META[spread].label}
              </option>
            ))}
          </select>
        </Field>
        <ToggleRow
          label="Show spread labels"
          checked={showLabels}
      ...
      <section className="panel grid gap-4">
        ...
        <ToggleRow
          label="Moon phase display"
          checked={moonPhase}
          onChange={setMoonPhase}
        />
        <ToggleRow
          label="New moon rituals"
          checked={newMoon}
          onChange={setNewMoon}
        />
      </section>
        ...
        <div className="grid grid-cols-5 gap-3">
          {CARD_BACKS.map((back) => (
            <button
              key={back.id}
              type="button"
              onClick={() => chooseBack(back.id)}
              className={cn(
                "flex flex-col items-center gap-2 rounded-2xl border p-2 transition",
                cardBack === back.id
            ...
            >
              ...
              <span className="font-display text-[11px] italic text-muted-foreground-strong">
                {back.label}
              </span>
            </button>
          ))}
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
          {[
            ...
            <button
              ...
              className={cn(
                "rounded-full border px-3 py-2 font-display text-sm italic transition",
                theme === value
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {BG_PRESETS.map((bg) => (
            <button
              key={bg.value}
              type="button"
              onClick={() => setPreset(bg.value)}
              className={cn(
                "rounded-xl border px-3 py-3 font-display text-sm italic transition",
                preset === bg.value
  ...
  return (
    <section className="panel grid gap-5">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Button type="button" onClick={exportData} className="gap-2 rounded-full">
      ...
      <button
        ...
        className="inline-flex items-center gap-2 font-display text-sm italic text-gold"
      >
        <RotateCcw className="h-4 w-4" /> Refresh settings
      </button>
    </section>
  );
}