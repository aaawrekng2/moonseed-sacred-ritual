...
export function MoonStreakIcon({ streakDays, size = 20, className }: Props) {
  ...
  const cx = 32;
  const cy = 32;
  // Offset a "background" circle to the right to eat the right portion
  // of the moon body, simulating waxing illumination from the left.
  const offset = (1 - fillRatio) * (r * 2);
  return (
    <svg
      viewBox="0 0 64 64"
      width={size}
